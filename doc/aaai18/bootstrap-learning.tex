...
This section formalizes the different learning tasks we address in the paper according to the amount of input knowledge available.


\subsubsection{Learning classical planning action models from (s,a,s') tuples}


\subsubsection{Learning classical planning action models from plans}
First we define the learning task of computing a planning action model from a given set of lifted predicates and a set of plans labelled with their corresponding initial and final states pairs. This task is defined as $\Lambda=\tup{\Psi,\Pi,\Sigma}$: 
\begin{itemize}
...
\item $I_{\Lambda}$ is extended with the fluents $plan(name(\xi),i,\Omega^{ar(a)})$, {\small $1\leq i\leq |\pi_1|$} that encode the plan $\pi_1\in \Pi$ for solving $P_1$, and the fluents $at_1$ and $\{next_{i,i_2}\}$, {\small $1\leq i<i2\leq n$}, for indicating that the plan step where to start validating the programmed model. The goals are like in the original compilation.
